...
def get_index_edges(A):
    num, n_nodes, _ = A.shape
    edge_index_list = []
    ...
    for i in range(num):
        ...
        # 提取非零边的源节点和目标节点
        edge_index = nonzero_indices.T  # 交换行列，使其成为 (2, num_edges) 的格式
        edge_index_list.append(edge_index)

    # 将所有图像块的边索引合并成一个张量，最终形状为 (num, 2, num_edges)
    edge_index_tensor = torch.stack(edge_index_list, dim=0)
